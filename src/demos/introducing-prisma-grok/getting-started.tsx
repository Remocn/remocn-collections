...
const typedCount = (frame: number, start: number, text: string) =>
  Math.max(0, Math.min(text.length, Math.floor((frame - start) * CHARS_PER_F)));
...
const CmdLine: React.FC<{
  ...
  const frame = useCurrentFrame();
  const n = typedCount(frame, start, text);
