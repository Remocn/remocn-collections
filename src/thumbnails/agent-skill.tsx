...
/**
 * The skill release told from the PROMPT side: the whole frame is the
 * `/remocn ›` terminal the video wears top-left for its full run, and the ask
 * is typed into it. The claim lands as the answer the agent gives back.
 */

// AI-company marks, verbatim from components/remocn/logo-enter.tsx
const CLAUDE_MARK =
...
const AgentChip: React.FC<{ bg: string; children: React.ReactNode }> = ({
  bg,
  children,
}) => (
  <div
...
export const AgentSkillThumb: React.FC = () => (
  <ThumbFrame background="#0e0d12">
    <AbsoluteFill>
      <div
        style={{
          position: "absolute",
          ...
          borderRadius: 9999,
          background:
            "radial-gradient(circle, rgba(195,232,141,0.16) 0%, rgba(14,13,18,0) 66%)",
          filter: "blur(30px)",
        }}
      />

      {/* the whole frame is the terminal the video wears */}
      <div
        style={{
          position: "absolute",
          left: 40,
      ...
      >
        <div
          style={{
            height: 78,
            flexShrink: 0,
            borderBottom: "2px solid rgba(242,242,242,0.12)",
            display: "flex",
            alignItems: "center",
            gap: 18,
            padding: "0 28px",
        ...
        >
          ...
          <div style={{ flex: 1 }} />
          <RemocnMark size={30} color="rgba(242,242,242,0.6)" />
        </div>
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {[
              { text: "Stops being a", color: REMOCN.ink },
              { text: "guessing game", color: REMOCN.lime },
            ].map((line) => (
              <div
                key={line.text}
                style={{
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              <AgentChip bg="#F0EEE6">
                <svg width={41} height={41} viewBox="0 0 256 257">
                  <path fill="#D97757" d={CLAUDE_MARK} />
                </svg>
              </AgentChip>
              <AgentChip bg="#0A0A0A">
                <svg width={39} height={39} viewBox="0 0 24 24" fill="#fff">
                  <path fillRule="evenodd" clipRule="evenodd" d={CODEX_MARK} />
                </svg>
              </AgentChip>
              <AgentChip bg="#0A0A0A">
                <svg width={36} height={42} viewBox="0 0 466.73 532.09" fill="#fff">
                  <path d={CURSOR_MARK} />
                </svg>
              </AgentChip>
              <AgentChip bg="#0A0A0A">
                <svg width={38} height={38} viewBox="0 0 1024 1024" fill="none">
                  <path fill="#fff" d={GROK_MARK_A} />
                  <path fill="#fff" d={GROK_MARK_B} />
                </svg>
              </AgentChip>
            </div>
            <span
